...
class HardwareRecommendationIT {
    ...
    fun shouldRecommendHardware() {
        ...
        try {
            ...
            val reports = zipReports(listOf(largeRecommendations, extraLargeRecommendations))
            LogManager.getLogger(javaClass.simpleName).info("Recommendations zipped in ${reports.toURI()}")
        } finally {
            cache.upload()
        }
    }
